...
class Property {
    ...
    public function bulkRemoveProperties( $demoOnly = false )
	{
		...
		$sql = "update {$wpdb->prefix}posts set `post_status` = 'trash' WHERE `post_type` = '" . self::THEME_POST_TYPE . "' AND `ID` IN ( SELECT `post_id` FROM {$wpdb->prefix}postmeta WHERE `meta_key` = '" . $metaKey . "' )";
	...
	*/
        ...
        if ( !empty( $idxIdentityFieldValue ) ){
			...
			if ( !empty( $wpdb ) ){
				...
				$totalsQuery =  "SELECT count(1) FROM `" . $wpdb->prefix . "postmeta` 
										WHERE	`meta_key` = '" . self::IDX_IDENTITY_FIELD . "_" . $idxIdentityFieldValue . "' AND
				...
				*/
				...
				$totalsQuery =  "SELECT COUNT(1) FROM `" . $wpdb->prefix . "posts` 
										WHERE	`post_status` <> 'trash' AND 
												`post_type` =  '" . self::THEME_POST_TYPE . "' AND
    ...
    private function insertPropertyMetas( $postId , $demo = false ){
        ...
        foreach ($this->customFields as $key => $value) {
			...
			if ( in_array( $value['slug'] , array( self::THEME_FIELD_PREFIX . 'prop_featured' ) ) ){
